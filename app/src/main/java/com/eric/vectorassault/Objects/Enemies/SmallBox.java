...
public class SmallBox extends EnemyObject
   {

      private Texture texture;
      int initialLaunch = 20;


      public SmallBox(Context context, PointF parentPosition)
         {
            super();
            ...
            texture = new Texture(context, R.drawable.box_glow3);
            ...
            // Set the location
            setWorldLocation(parentPosition.x, parentPosition.y);

            setMaxSpeed(r.nextInt(10)+40);

            setSpeed(getMaxSpeed());


            // Make sure we know this object is a ship
            setType(Type.SMALL_BOX);
      ...
      public void update(long fps, PointF playerPos, boolean avoid, Bullet bullet)
         {
            if(initialLaunch == 0)
               ...
               {
                  setxVelocity((float) (getSpeed() * Math.cos(Math.toRadians(travellingAngle))));
                  setyVelocity((float) (getSpeed() * Math.sin(Math.toRadians(travellingAngle))));
